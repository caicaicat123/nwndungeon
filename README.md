...
在世界上随机生成副本入口（带标记的铁门 + 石压力板），踩下压力板后把门口一个区块内的玩家一起传送到独立的副本世界。

## 核心规则

- **入口是铁门 + 石压力板**（铺在门前地板上，走近门就会踩到），只有插件登记过的门才会触发；玩家自己放的门是普通门
- **踩板触发 → 读条进本**：踩下压力板后亮起 3 秒进度条（带粒子 + 音效），读条期间跑远（默认 6 格）/ 死亡 / 掉线会取消本次进入；
